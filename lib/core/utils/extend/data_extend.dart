...
extension DataExtendNum on num? {
  ...
  String toAreaUnit() {
    return '${PriceExtend.strToNumberFormat(this.toString()) ?? 0} ${AppConstant.AREA_UNIT}';
  }
  ...
  String toThousandSeparator() {
    return '${PriceExtend.strToNumberFormat(toString())}';
  }
...
extension DataExtendString on String {
  String? toPeriodStr() {
    ...
    return (period.length) < 6 ? period : '${period.substring(0, 2)}/${period.substring(2, 6)}';
  }
...
extension DataExtendStringNullAble on String? {
  ...
  String? get toBiPoint => '${(this ?? '0').toThousandSeparator()} ${AppConstant.BI_WALLET_UNIT}';
  ...
  String? toThousandSeparator() {
    return isNotNullOrEmpty() ? '${PriceExtend.strToNumberFormat(this)}' : null;
  }

  String? removeThousandSeparator() {
    return isNotNullOrEmpty() ? '${PriceExtend.strToNumberParse(this)}' : null;
  }
  ...
  String? toAreaUnit() {
    ...
    return '${PriceExtend.strToNumberFormat(this)} ${AppConstant.AREA_UNIT}';
  }

  String? withAreaUnit() {
    if (this.isNullOrEmpty()) return null;

    return '${this} ${AppConstant.AREA_UNIT}';
  }
