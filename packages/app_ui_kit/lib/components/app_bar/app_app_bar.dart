...
class AppAppBarTitleText extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return title.textAuto.textStyle(context.themeText.titleLarge).minFontSize(FontSizeService().text_S).maxLines(args?.maxLine ?? 1).align(args?.textAlign ?? TextAlign.start).color(args?.color).ellipsis.make();
  }
}
